...
def execute_query(query_string: str, convert_to_dict=True):
    ...
    with sqlite3.connect("CookBookDatabase.db") as con:
        ...
        if convert_to_dict:
            ...
            # if not requested in Dict form, return as a List, which is readily combobox-compatible if defined correctly
            print(result)
            return result
...
def execute_insert_script(script_string: str, table_name: str = None, id_column: str = None):
    ...
    with sqlite3.connect("CookBookDatabase.db") as con:
        ...
        # if a table name was specified, then the calling coded is requesting a newly generated ID to be reported back
